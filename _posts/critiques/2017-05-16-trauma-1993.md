...
[movie]A Nightmare on Elm Street|1984[/movie]

{% assign text = "A Nightmare on Elm Street|1984" %}
{% assign movie = text | split: '|'%}
{% assign posts = site.posts | where: "title", movie[0] %}
{% assign post = posts | where: "year", movie[1] | first %}
{% if post %}[{{ post.title }}]({{ post.url }}){% else %}{{ movie[0] }}{% endif %}

{% assign movie = "A Nightmare on Elm Street|1984" %}
{% assign title = movie | split: '|' | first %}
{% assign year = movie | split: '|' | last %}
{% assign posts = site.posts | where: "title", title %}
{% assign post = posts | where: "year", year | first %}
{% if post %}[{{ post.title }}]({{ post.url }}){% else %}{{ title }}{% endif %}

{% assign movie = "A Nightmare on Elm Street" }
{% assign post = site.posts | where: "title", {{ movie }} | first %}
{% if post %}[{{ post.title }}]({{ post.url }}){% else %}{{ movie }}{% endif %}
